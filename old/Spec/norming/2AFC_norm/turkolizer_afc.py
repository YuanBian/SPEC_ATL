...
def make_header(final):
    ...
    for i in range(len(final[0])):
        header.append("optA__" + str(i+1))
        num_questions = len(final[0][i].questions)
        for j in range(num_questions):
            header.append("optB" + "__" + str(i+1))
...
def randomize(L, F, Y):
    ...
    while True:
        ...
        if IsCloseSqueeze:
            ...
            totalW = sum(w for _, w in weights)
            #print "totalW:", totalW
            if fillerIndex != None:
...
def finalize(lsts, num_fillers_inbetween, num_fillers_starting):
    """ Randomize each experiment list, and from them create final versions of
    the lists of the Trial objects. """
    for lst in lsts:
        for x in lst:
            random.shuffle(x)
    return [randomize(lst, num_fillers_inbetween, num_fillers_starting) for lst in lsts]
