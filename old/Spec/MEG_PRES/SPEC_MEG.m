...
pwd = '/Users/yuanbian/Documents/Spec_ATL/Spec/MEG_PRES';
...
practiceToSave = [pwd '/data/SPEC_' 'subject' subjID '_practice.csv'];
...
practice_results.ISI = {ISI;ISI;ISI;ISI};
...
try
    for runIndex = 1:NUM_RUN
        ...
        for trialIndex=1:TRIALS_PER_RUN
            ...
            %send_trigger(p, trigger_idx{runIndex, trialIndex},0.004);

            ISI = ISIs{runIndex, trialIndex};
            ...
            %word (1000ms)
            PTBhelper('stimText', wPtr, noun_stims{runIndex, trialIndex}, fixFontSize);
            WaitSecs(WORD_DUR);
            ...
            %question (max 3,000ms)
            PTBhelper('stimText', wPtr, ques_stims{runIndex, trialIndex}, fixFontSize);
        ...
        runFile = sortedFiles(runIndex);
        run = readtable(runFile);
        results.Noun = reshape(noun_stims(runIndex, :),TRIALS_PER_RUN,1);
        results.Question = reshape(ques_stims(runIndex, :),TRIALS_PER_RUN,1);
