...
try
        ...
        fileName = fileName{1};
        practiceFileToSave = [pwd '/data/REDBOAT_MEG_' subj_ID '_subject' subj_num '_practice.csv'];
        writetable(results, practiceFileToSave);
        %results = [];
        disp(strcat('Subj', subj_ID, ' finished; data for this run saved to ', practiceFileToSave))
...
% PTBhelper('waitUntil',GetSecs+INSTRUCT_DUR,kbIdx,escapeKey);%INSTRUCT_DUR
% WaitSecs(1);
...
%         block = [];
        ...
        fileName = fileName{length(fileName)};
        fileToSave = [pwd '/data/REDBOAT_MEG_' subj_ID '_' fileName];
    ...
    %ShowCursor;
     ...
     %Save all data
%     fileToSave = [pwd '/data/FAILED_REDBOAT_MEG_' subj_ID '_' fileName];
%     writetable(results, fileToSave);
